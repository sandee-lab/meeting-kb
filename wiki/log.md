...
- 마크다운 테이블 → 리스트 형식 전환 (헤더/구분선 제거)
- 프로젝트 개요 섹션 제거 (참여자 기지정보)
- 전체 index 변화: 328줄(원본) → 89줄(1단계) → 60줄(2단계), 토큰 ~80% 절감

## [2026-04-07] structure | index.md 2단계 라우팅 구조 분리

- 목적: Query당 토큰 소모 절감 (index 13k → 라우터 ~3k + 필요한 서브인덱스 1개)
- 변경사항:
  - index.md → 슬림 라우터로 재작성 (wiki 문서 키워드 테이블 + 서브인덱스 포인터)
  - index-meetings.md 신규 — 회의록 85건 서브인덱스
  - index-feedback.md 신규 — 피드백 24건 + 퍼블리셔 문서 서브인덱스
  - CLAUDE.md Query 패턴에 2단계 라우팅 규칙 추가
- 기획서: 기존 raw/gdd/gdd_index.csv를 그대로 활용 (별도 서브인덱스 불필요)

## [2026-04-07] query | 6월까지 vs 7~8월 빌드 특징 비교 (피드백 포함)

- 질문: "6월까지의 빌드와 7~8월 빌드의 특징 설명" + 유저 피드백 포함 재답변
- 참조 문서: concepts/빌드-진화-단계, timeline/2025-상반기, timeline/2025년-하반기, concepts/플레이테스트-유저피드백, raw/feedback 6~8월 4회치
- 핵심 합성: 6월 피드백은 "방향은 맞는데 미완성(운빨·연출·시인성)"; 7월은 "구조 자체 재검토 수준" → 방향성 확정 직접 촉발; 8월은 "방향 됐고 완성도" 단계로 피드백 성격 변화
- 발견: 7/11 피드백(웨이브제·유닛 2종 제한 제안)이 7월 방향성 확정에 직접 반영된 사실이 원본 확인으로 명확해짐
- wiki 보완 완료: concepts/플레이테스트-유저피드백.md 6~8월 4개 섹션 상세 보완 — 운빨 부재 직접 인용, 7/11 피드백→방향성 확정 인과관계 명시, 회피 답답함·재화 쾌감 소실 추가, 반복 이슈 표에 "운빨 경험 부재" 항목 신설

## [2026-04-07] ingest | 마일스톤 기획 문서 10건 (M2~M7, [2503]M1)

- 신규 원본 10건 등록: M2_4월·M3_5월·M4_6월·M5_7월·M6_8월·M6_8월_2·M7_9월·M7_2_9월·[2503]M1_로그라이트버전·[2503]M1_캐릭터중심
- index.md: 회의록 수 75→85, 총 원본 217→227로 갱신; 4월·5월 신규 섹션 추가, 3·6·7·8·9월 섹션 보완
- timeline/2025-상반기.md: 3월 [2503]M1 추가, 4월 M2 새 섹션, 5월 M3 새 섹션, 6월 M4 내용 보완, 7월 M5 내용 보완
- timeline/2025년-하반기(8월~12월).md: 8월 M6/M6_2 추가(등급 강화·보스 연출), 9월 M7/M7_2 추가(전투 템포·보스 타임어택·스폰 시스템)
- concepts/인게임-전체설계.md: M2 멀티 유닛 컨트롤·각성 시스템 섹션 추가, M7 완료 목록 보완, M6 연출 강화 섹션 추가
- concepts/택틱 시스템.md: M5 Add_Skill 시스템·택틱 상점 섹션 추가, [2503]M1 히든 택틱 상세 보완
