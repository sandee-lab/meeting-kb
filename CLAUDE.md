...
### Query (Two Outputs)
1. **1단계 — 검색**: `Grep pattern="키워드" path="wiki/" files_with_matches head_limit=0` → 매칭 파일 전체 목록
   - wiki/는 55개 이하이므로 limit 없이 전체 반환
   - 파일명으로 핵심 2~3개를 특정할 수 있으면 → 바로 직독
2. **1-1단계 — 보조** (판단 불확실 시): index.md 라우터의 도메인코드+요약+관계를 참조하여 문서 선택
   - ⚠️ index.md에서 microindex·다른 인덱스로 순차 탐색하지 않는다
3. **2단계 — 직독**: 특정된 wiki/ 문서를 읽어 답변 구성
4. **3단계 — 보충** (필요시): wiki/에 부족하면 → Grep raw/ → 원본 직독. 마이크로인덱스는 필요한 도메인 1개만
5. 새로운 합성이 나왔으면 → **wiki 업데이트** + log.md 기록
